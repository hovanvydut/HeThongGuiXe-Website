...
public class PaymentDAOImpl implements PaymentDAO {
	...
	public Optional<Payment> getCurrentPayment(Customer customer) {
		...
		try {
			...
			cq.select(root);
			cq.where(cb.and(cb.equal(root.get(Payment_.customer), customer),
	...
	@Override
	public List<Payment> getAllUnpaidPayment(Customer customer) {
		EntityManager em = EntityManagerUtil.getInstance().getEntityManager();
		...
		try {
			...
			cq.select(root);
			cq.where(cb.and(cb.equal(root.get(Payment_.customer), customer), cb.isNull(root.get(Payment_.paid_at))));
